...
function createAccordionList(cks, callback, callbackArguments) {
    ...
    for (var i = 0; i < cks.length; i++) {
        ...
        var titleElement = $("<div/>").addClass('collapsible-header').append($("<a/>").html(titleText.html()).attr("href", "#"));
        let itemLength = $('#cookiesList .cookie').length;
    ...
    $(".deleteOne").click(function () {
        ...
        var okFunction = function () {
            ...
            deleteCookie(url, name, storeId, function (success) {
                if (success === true) {
                    var head = cookie.prev('div.collapsible-header');
